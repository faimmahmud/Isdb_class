...
function db_config(): array {
    return [
        'host' => getenv('AURELIA_DB_HOST') ?: '127.0.0.1',
        'port' => getenv('AURELIA_DB_PORT') ?: '3306',
        'name' => getenv('AURELIA_DB_NAME') ?: 'aurelia_travel',
        'user' => getenv('AURELIA_DB_USER') ?: 'root',
...
function database_install(bool $fresh = false): array {
    ...
    $server->exec('CREATE DATABASE IF NOT EXISTS ' . db_identifier($config['name']) . ' CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');

    $pdo = db_connect($config, true);
    $GLOBALS['aurelia_pdo'] = $pdo;

    if ($fresh) {
        foreach (['SET FOREIGN_KEY_CHECKS = 0', 'DROP TABLE IF EXISTS package_features', 'DROP TABLE IF EXISTS bookings', 'DROP TABLE IF EXISTS packages', 'DROP TABLE IF EXISTS users', 'SET FOREIGN_KEY_CHECKS = 1'] as $sql) {
            $pdo->exec($sql);
        }
    }
...
function normalize_booking_row(array $row): array {
    ...
    foreach (['id', 'booking_ref', 'booking_type', 'package_id', 'package_name', 'country', 'departure_from', 'destination', 'travel_date', 'travel_time', 'leave_date', 'leave_time', 'customer_name', 'customer_email', 'customer_phone', 'payment_method', 'payment_reference', 'payment_status', 'booking_status', 'currency', 'message', 'booked_by', 'booked_role', 'booking_channel', 'ip_address', 'user_agent', 'created_at', 'updated_at'] as $key) {
...
function save_booking_row(PDO $pdo, array $booking, bool $upsert = true): bool {
    $booking = normalize_booking_row($booking);
    if ($booking['id'] === '' || $booking['package_name'] === '' || $booking['customer_name'] === '' || $booking['customer_email'] === '' || $booking['customer_phone'] === '') {
        return false;
    }
    ...
    $sql = '
        INSERT INTO bookings (
            ...
            payment_method, payment_reference, payment_status, booking_status, amount, currency, message,
            booked_by, booked_role, booking_channel, ip_address, user_agent, created_at, updated_at
        ) VALUES (
            :id, :booking_ref, :booking_type, :package_id, :package_name, :country, :departure_from, :destination,
            :travel_date, :travel_time, :leave_date, :leave_time, :guests, :customer_name, :customer_email, :customer_phone,
            :payment_method, :payment_reference, :payment_status, :booking_status, :amount, :currency, :message,
