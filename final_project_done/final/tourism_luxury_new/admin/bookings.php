...
function badge_html(string $text, string $class = 'secondary'): string {
    return '<span class="badge rounded-pill bg-' . $class . '">' . e($text) . '</span>';
}
    ...
    <div class="surface p-4 p-lg-5 rounded-5 reveal mb-4">
      <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
        <div>
          ...
          <h1 class="section-title mb-2">Customer booking details</h1>
          <p class="section-lead mb-0">Search by name, package, email, or reference. Track travel date, leaving time, and payment method.</p>
        </div>
      ...
      <form class="row g-3 align-items-end" method="get">
        ...
        <div class="col-md-4 col-lg-2">
          ...
          <select name="payment" class="form-select">
            <?php foreach (['all','pending','paid','refunded','failed'] as $opt): ?>
              <option value="<?= e($opt) ?>" <?= $payment === $opt ? 'selected' : '' ?>><?= e(ucfirst($opt)) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-4 col-lg-2">
          ...
          <select name="type" class="form-select">
            <?php foreach (['all','package','ticket','tour'] as $opt): ?>
          ...
          <tbody>
            <?php foreach ($bookings as $b): ?>
              <tr>
                ...
                <td>
                  <strong><?= e($b['leave_date'] ?: '—') ?></strong><br>
                  <span class="text-muted small"><?= e($b['leave_time'] ?: '—') ?></span>
                </td>
                <td>
                  <strong><?= e(ucfirst($b['payment_method'])) ?></strong><br>
                  <span class="text-muted small"><?= e($b['payment_reference'] ?: 'No ref') ?></span><br>
                  <?= badge_html(ucfirst($b['payment_status']), status_class($b['payment_status'])) ?>
                </td>
                <td>
                  <?= badge_html(ucfirst($b['booking_status']), status_class($b['booking_status'])) ?><br>
